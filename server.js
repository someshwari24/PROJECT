...
app.get('/me', (req, res) => {
    ...
    try {
      ...
      User.findById(decoded.userId)
        .select("name username email") // Only return necessary fields
        .then(user => {
...
app.post('/submit-details', authMiddleware, async (req, res) => {
    ...
    console.log("Received Data:", { name, address, city, state, pincode, mobile, item, language, date, time });
...
async function getAssistantDetails(city, state) {
    try {
        if (!city || !state) {
            console.log("❌ Invalid city/state input:", city, state);
        ...
        console.log("🔍 Searching assistant for:", `"${trimmedCity}"`, `"${trimmedState}"`);
        ...
        if (!assistant) {
            console.log("❌ No assistant found in", trimmedCity, trimmedState);
